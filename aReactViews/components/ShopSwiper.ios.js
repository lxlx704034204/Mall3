...
var midHeight = screenWidth*0.37037037;
...
var leftSidePosition = -sidewidth;//左边view位置

// var rightSidePosition = screenWidth-midPosition;//右边view位置
var rightSidePosition = screenWidth;//右边view位置
...
var moveDDistance = 25;//判断移动间隔左边移动距离
...
//ios轮播请求
      _iosFetchForADs(){
        console.log('_iosFetchForADs');
        fetch(URL_SLIDER_AD).then(function(res) {
            if (res.ok) {
              console.log('fetchad------------获取成功');
              that.timer && clearTimeout(that.timer);
              res.json().then(function(data) {
                that._initDatas(data);
              });
            } else {
              console.log("fetchad------------, got status", res.status);
            }
          }.bind(this), function(e) {
            console.log("fetchad------------Fetch failed!", e);
          }.bind(this));
      }
...
//  更新数据
    ...
    componentWillMount(){
      ...
      if (Platform.OS === 'ios') {
        ...
        URL_SLIDER_AD = Config.homeSliderURL;
        // that._iosFetchForADs();
      ...
      var ContentView = maxDatasSize>1?(<View><View style={styles.container}>
                                        ...
                                        <Animated.View style={{
                                             ...
                                             {imgs.map((item) => (
                                               ...
                                               <View style={[styles.tagStyle,{backgroundColor:'#C72229'}]}></View>:
