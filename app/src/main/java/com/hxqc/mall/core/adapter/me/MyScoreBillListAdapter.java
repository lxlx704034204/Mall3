 ...
 * Todo 账单列表的适配器
 */
public class MyScoreBillListAdapter extends MyBillListAdapter {
    ...
    public MyScoreBillListAdapter(VRecyclerViewFooter vRecyclerViewFooter,
                ...
                for (int i = 0; i < mData.size(); i++) {
                    ...
                    if (list.matter.size() > 0)
                        for (ScoreBillMatter matter : list.matter) {
                            ...
                            map.put("prepaidMonth", list.obtainMonth);
                            groupName.add(map);
...
//        DebugLog.e("getHeaderId", groupName.toString());
    ...
    public void onBindHeaderViewHolder(RecyclerView.ViewHolder holder, int position) {
        if (position < getItemCount() - 1) {
            ...
            headHolder.groupItem.addData(map, mContext.getString(R.string.score));
    ...
    public void onVBindViewHolder(RecyclerView.ViewHolder holder, final int position) {
        switch (getItemViewType(position)) {
            case TYPE_HEAD:
                ...
                headerHolder.headItem.addData(scoreBill);
                break;
            case TYPE_NORMAL:
