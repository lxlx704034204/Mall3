...
public class StickyHeadersTouchListener implements RecyclerView.OnItemTouchListener {
    ...
    public StickyHeadersTouchListener(RecyclerView parent,
                                      HeaderStore headerStore) {
        this.headerStore = headerStore;
        this.gestureDetector = new GestureDetector(parent.getContext(),
                new SingleTapDetector(parent));
    }
    ...
    private class SingleTapDetector extends GestureDetector.SimpleOnGestureListener {
        ...
        public boolean onSingleTapConfirmed(MotionEvent e) {
            ...
            RecyclerView.ViewHolder holder = findItemHolderUnder(e.getX(),
                    e.getY());
            if (holder != null) {
                View headerView = headerStore.getHeaderViewByItem(holder);
                long headerId = headerStore.getHeaderId(holder.getPosition());
                listener.onHeaderClick(headerView, headerStore.headersViewByPosition.get(headerId),
                        headerId);
                return true;
            }
        ...
        private RecyclerView.ViewHolder findItemHolderUnder(float x, float y) {
            ...
            for (int i = parent.getChildCount() - 1; i > 0; i--) {
                ...
                if (holder != null && headerStore.isHeader(holder)) {
                    if (y < item.getTop()
                            && item.getTop()
                            - headerStore.getHeaderHeight(holder) < y) {
                        return holder;
                    }
                }
            }
