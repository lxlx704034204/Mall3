...
public class OverlayDrawer extends DraggableDrawer {
    ...
    private boolean mSidewardCloseMenu = true;//菜单外部点击事件
    ...
    public boolean onInterceptTouchEvent(MotionEvent ev) {
        if (mTouchMode == TOUCH_MODE_FULLSCREEN_MENUOPEN && !mMenuVisible) {
            //菜单打开之后可以拖动，关闭不可自行拖出
            return false;
        }
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_MOVE: {
                ...
                if (checkTouchSlop(dx, dy)) {
                    if (mOnInterceptMoveEventListener != null && (mTouchMode == TOUCH_MODE_FULLSCREEN || mMenuVisible) && canChildrenScroll((int) dx, (int) dy, (int) x, (int) y)) {
    ...
    public boolean isSidewardCloseMenu() {
        return mSidewardCloseMenu;
    }

    public void setSidewardCloseMenu(boolean sidewardCloseMenu) {
        this.mSidewardCloseMenu = sidewardCloseMenu;
    }

    protected boolean onDownAllowDrag(int x, int y) {
        switch (getPosition()) {
            case LEFT:
                return (!mMenuVisible && mInitialMotionX <= mTouchSize) || (mMenuVisible && mInitialMotionX <= mOffsetPixels);

            case RIGHT:
                final int width = getWidth();
                final int initialMotionX = (int) mInitialMotionX;

                return (!mMenuVisible && initialMotionX >= width - mTouchSize) || (mMenuVisible && initialMotionX >= width + mOffsetPixels);
            ...
            case BOTTOM:
                final int height = getHeight();
                return (!mMenuVisible && mInitialMotionY >= height - mTouchSize) || (mMenuVisible && mInitialMotionY >= height + mOffsetPixels);
        }
    ...
    protected boolean onMoveAllowDrag(int x, int y, float dx, float dy) {
        ...
        switch (getPosition()) {
            ...
            case RIGHT:
                final int width = getWidth();
                return (!mMenuVisible && mInitialMotionX >= width - mTouchSize && (dx < 0)) || (mMenuVisible && x >= width - mOffsetPixels) || (Math.abs(mOffsetPixels) <= mPeekSize && mMenuVisible);

            case TOP:
                return (!mMenuVisible && mInitialMotionY <= mTouchSize && (dy > 0)) || (mMenuVisible && x <= mOffsetPixels) || (Math.abs(mOffsetPixels) <= mPeekSize && mMenuVisible);

            case BOTTOM:
                final int height = getHeight();
                return (!mMenuVisible && mInitialMotionY >= height - mTouchSize && (dy < 0)) || (mMenuVisible && x >= height - mOffsetPixels) || (Math.abs(mOffsetPixels) <= mPeekSize && mMenuVisible);
        }
