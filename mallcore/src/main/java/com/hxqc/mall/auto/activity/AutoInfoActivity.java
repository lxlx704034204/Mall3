...
public class AutoInfoActivity extends BackActivity implements SwipeMenuListView.OnMenuItemClickListener, AdapterView.OnItemClickListener, View.OnClickListener, OnRefreshHandler {
...
//                    AutoInfoControl.getInstance().requestAutoInfo(this, this);
                    refreshData(autoGroups);
                    AutoInfoControl.getInstance().requestAutoInfo(AutoInfoActivity.this, autoInfoCallBack);
                } else {
                    //获取本地车辆数据
//                    ArrayList<MyAuto> autoDataLocal = MyAutoInfoHelper.getInstance(this).getAutoDataLocal();
        ...
        public void onSuccess(ArrayList<MyAuto> response) {
            ...
            if (response != null && !response.isEmpty()) {
                if (flags == AutoInfoContants.RESERVE_MAINTAIN) {
                    ...
                    autoGroups = response;
                    AutoHelper.getInstance().createAutoLocal(AutoInfoActivity.this, response, AutoHelper.AUTO_INFO);
                    DebugLog.i(TAG, "onAutoInfoSucceed 有数据");
                    DebugLog.i(TAG, response.get(0).toString());
                    DebugLog.i(TAG, "size:" + response.size());
//                ArrayList<MyAuto> autoLocal = AutoHelper.getInstance().getAutoLocal(AutoInfoActivity.this, AutoHelper.AUTO_LOCAL_INFO);
...
/*if (response != null && !response.isEmpty()) {
                    ...
                    AutoHelper.getInstance().createAutoLocal(AutoInfoActivity.this, response, AutoHelper.AUTO_LOCAL_INFO);
...
//            AutoHelper.getInstance().createAutoLocal(AutoInfoActivity.this, autoGroups, AutoHelper.AUTO_LOCAL_INFO);
            AutoInfoControl.getInstance().requestAutoInfo(AutoInfoActivity.this, autoInfoCallBack);
//        refreshData(autoGroups);
        }
    ...
    private void NoneDataListView() {
        if (flags == AutoInfoContants.AUTO_DETAIL) {
            ...
            mRequestFailView.setAddButtonClick().setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View v) {
                    ActivitySwitchAutoInfo.toAddOrEditAutoInfo(AutoInfoActivity.this, null, shopID, flags, AutoInfoContants.ADD_PAGE);
                }
            });
        } else if (flags == AutoInfoContants.RESERVE_MAINTAIN) {
            mRequestFailView.setVisibility(View.VISIBLE);
            ...
            mNoDataBtnView.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View v) {
//                    DebugLog.i(TAG,"去保养");
                    ActivitySwitchAutoInfo.toAddOrEditAutoInfo(AutoInfoActivity.this);
                }
            });
        }
...
/*  if (autoInfoAdapterV3 == null) {
            autoInfoAdapterV3 = new AutoInfoAdapterV3(AutoInfoActivity.this, new ArrayList<MyAuto>());
            mAutoInfoListView1.addFooterView(mAutoInfoFootView, null, false);
...
/**
    ...
    private void refreshData(ArrayList<MyAuto> myAutos) {
        */
/*if (autoInfoAdapterV2 == null) {
            autoInfoAdapterV2 = new AutoInfoAdapterV2(AutoInfoActivity.this, myAutos);
//            mAutoInfoListView1.addFooterView(mAutoInfoFootView, null, false);
        ...
        if (autoInfoAdapterV3 == null) {
            autoInfoAdapterV3 = new AutoInfoAdapterV3(AutoInfoActivity.this, myAutos);
//            mAutoInfoListView1.addFooterView(mAutoInfoFootView, null, false);
            mAutoInfoListView1.setAdapter(autoInfoAdapterV3);
        } else {
    ...
    private CallBack<ArrayList<BrandGroup>> brandGroupCallBack = new CallBack<ArrayList<BrandGroup>>() {
        ...
        public void onSuccess(final ArrayList<BrandGroup> brandGroups) {
            AutoInfoControl.getInstance().requestAutoInfo(AutoInfoActivity.this, new CallBack<ArrayList<MyAuto>>() {
                @Override
                public void onSuccess(ArrayList<MyAuto> response) {
                    //检索是否与是店铺车辆
                    autoGroups = new ArrayList<MyAuto>();
...
/*
    private void dialogAutoInfoComplete() {
        View view = LayoutInflater.from(AutoInfoActivity.this).inflate(com.hxqc.mall.core.R.layout.view_auto_info_complete, null);
        final AlertDialog imAlertDialog = new AlertDialog.Builder(AutoInfoActivity.this)
                .setView(view)
                .create();
        ...
        completeView.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                if (UserInfoHelper.getInstance().isLogin(AutoInfoActivity.this)) {
                    ActivitySwitchAutoInfo.toAutoInfo(AutoInfoActivity.this);
                    imAlertDialog.dismiss();
                }
            }
        });
