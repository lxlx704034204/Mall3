    ...
    public MyCoordinatorLayout(final Context context, AttributeSet attrs) {
        ...
        this.post(new Runnable() {
            ...
            public void run() {
                ...
                /** 为不同的Activity预留回调接口，用于控制recycleview的下拉控制条件 **/
                if (callBack != null) callBack.setRecyclerViewPullCondition(recyclerView_top);

//                if (context instanceof FilterThirdShopActivity_2) {
//                    ((FilterThirdShopActivity_2)context).setLimitCondition(recyclerView_top);
//                }
...
//        DebugLog.e(TAG, this.getChildAt(0).getTop() + "　" + this.getChildAt(0).getBottom() + " \n" + this.getChildAt(1).getTop() + " " + this.getChildAt(1).getBottom());
    ...
    public interface CallBack {
        /** MyCoordinatorLayout中设置recyclerView触发搜索框显示的条件 */
        void setRecyclerViewPullCondition(int pullCondition);
    }
}
