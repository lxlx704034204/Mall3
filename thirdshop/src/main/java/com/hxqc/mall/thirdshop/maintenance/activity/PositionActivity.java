...
public class PositionActivity extends BasePositionActivity {
    ...
    protected void getData() {
        if (areaCacheUtil.isExistCache(AreaCacheUtil.MAINTENANCE) && baseSharedPreferencesHelper.getLoadPosition()) {
            locationAreaModels = areaCacheUtil.getCache(AreaCacheUtil.MAINTENANCE);
            ...
            ThirdPartShopClient.requestFilterArea(new DialogResponseHandler(this) {
                ...
                public void onSuccess(String response) {
                    ...
                    if (locationAreaModels == null || locationAreaModels.size() <= 0) {
                        if (areaCacheUtil.isExistCache(AreaCacheUtil.MAINTENANCE)) {
                            locationAreaModels = areaCacheUtil.getCache(AreaCacheUtil.MAINTENANCE);
                        }
                        ExpandAdapter myAdapter = new ExpandAdapter(locationAreaModels);
                        location_expand_list_view.setAdapter(myAdapter);
                    ...
                    areaCacheUtil.dataChangeAndUpdate(AreaCacheUtil.MAINTENANCE, areaCacheUtil.transformData(AreaCacheUtil.MAINTENANCE, locationAreaModels));
                    int i = 0;
