...
public class CarSeriesRecyclerViewAdapter extends RecyclerView.Adapter {
    ...
    public ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        ...
        if (viewType == IS_HEADER) {
            ...
            View view = LayoutInflater.from(mContext).inflate(R.layout.t_car_series_recyclerview_item, parent, false);
    ...
    public void onBindViewHolder(RecyclerView.ViewHolder holder, int position) {
        ...
        if (position == 0 && ((ViewHolder) holder).viewType == IS_HEADER) {
            ...
            if (position - 1 >= 0 && position - 1 < series.size()) {
                ...
                ImageUtil.setImage(mContext, ((ViewHolder) holder).mCarSeriesImageView, series.seriesThumb);
